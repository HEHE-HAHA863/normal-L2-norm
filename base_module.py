...
class Encoder(nn.Module):
    def __init__(self, isize, nc, k=100, ndf=64):
        ...
        main = nn.Sequential()
        main.add_module('initial_{0}_{1}_convt'.format(nc, ndf),
        ...
        while csize > 4:
            in_feat = cndf
            out_feat = cndf * 2
            main.add_module('pyramid_{0}_{1}_convt'.format(in_feat, out_feat),
        ...
        main.add_module('final_{0}_{1}_convt'.format(cndf, 1),
...
class Decoder(nn.Module):
    def __init__(self, isize, nc, k=100, ngf=64):
        ...
        main = nn.Sequential()
        main.add_module('initial_{0}_{1}_convt'.format(k, cngf), nn.ConvTranspose2d(k, cngf, 4, 1, 0, bias=False))
        ...
        main.add_module('final_{0}_{1}_convt'.format(cngf, nc), nn.ConvTranspose2d(cngf, nc, 4, 2, 1, bias=False))
